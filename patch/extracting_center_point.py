...
def click_and_crop(event, x, y, flags, param):
    ...
    global refPt, thickPt, cropping

    # 왼쪽 마우스가 클릭되면 (x, y) 좌표 기록을 시작하고
    # cropping = True로 만들어 줍니다.


    # Center position을 가져옴
    # 0: ROI영역을 참조하여 Center position을 선택
    # 1: 반지금을 입력해 동공부분 표시
    ...
    if event == cv2.EVENT_LBUTTONDOWN:  # center position
        ...
        print("center end")
        refPt.append((x, y))
        cv2.line(image, refPt[0], distance(refPt[0], refPt[0]), (160, 160, 160), 2)
...
if __name__=='__main__':
    ...
    PATH_RESULT = '/mnt/2TBDisk/WORK/Broadbeam_bluelight/result'
    PATH_ORIGINAL = '/mnt/8TBDisk/BRC/BRC2019_Q_test'
    ...
    # =============================================================
    label_width = 3
    for nPatient in [list_patient for list_patient in os.listdir(PATH_ORIGINAL) if
                     not list_patient.startswith('.')]:  # iterate all patient
        print(nPatient)
        for nCase in [list_case for list_case in os.listdir(os.path.join(PATH_ORIGINAL, nPatient)) if
            ...
            for nQ in [list_Q for list_Q in os.listdir(os.path.join(PATH_ORIGINAL, nPatient, nCase, series_Q)) if
                       list_Q.endswith('.dcm')]:  # iterate all Q(dcm) file
                ...
                if "GraphicAnnotationSequence" in dcm_q:
                    for nAnnotation in range(len(dcm_q.GraphicAnnotationSequence)):
                        ...
                        if isDrawn == True:
                            ...
                            draw_img_name = os.path.join(PATH_ORIGINAL, nPatient, nCase, series_Q,
                                                         nQ.split('_')[3].split('.')[0] + "-" + name_series.split('.')[
                                                             0] + "-" + name_dcm.replace('.dcm', '.png'))
